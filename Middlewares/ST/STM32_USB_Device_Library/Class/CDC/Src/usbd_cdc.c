...
uint8_t  USBD_CDC_TransmitPacket(USBD_HandleTypeDef *pdev,  uint16_t index)
{
  ...
  if (pdev->pClassData != NULL)
  {
    if (hcdc->TxState == 0U)
    {
      ...
      /* Update the packet total length */
      pdev->ep_in[((index < 2) ? CDC_IN_EP : (CDC_IN_EP+2)) & 0xFU].total_length = hcdc->TxLength;
      /* Transmit next packet */
      USBD_LL_Transmit(pdev, (index < 2) ? CDC_IN_EP : (CDC_IN_EP+2), hcdc->TxBuffer,
                       (uint16_t)hcdc->TxLength);
				#ifdef USB_LOG
				printf("USBD_CDC_TransmitPacket transmit OK\n\r");
				#endif
      return USBD_OK;
    }
    else
